...
.. code-block:: pytest
    ...
    $ pytest -vv tests/test_task_fails.py
    ============================= test session starts ==============================
    …
    collected 1 item
...
.. code-block:: pycon
    ...
    $ uv run python tests/test_task_fails.py
...
check. For example, the ``Task`` data class is set up so that two tasks with
...
the Tasks API, which has an ``TasksDB`` class that requires a path argument.

.. code-block:: python

    from tasks.api import TasksDB
...
trying to initialise the custom ``TasksDB`` type. We can write a test to ensure
